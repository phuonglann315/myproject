...
<section class="new_arrivals section-padding-03">
    <div class="container">
        ...
        <div class="section-title text-center">
            <h4 class="title text_darkt">Special Offers</h4>
            <hr>
            <span class="title-subtitle text_light"><strong></strong></span></div>
        <!-- Section Title End -->
        <div class="page-banner offer-page-banner section">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <ul class="nav nav-tabs justify-content-center text-uppercase" id="myTab" role="tablist"
                    ...
                    from (select IDPR,NAMEPR,CREATEDATE,QUANTITY, 
...
//                    $param =[$_GET['IDBR']];
                    ...
                    $query = "select * from ( select *, ROW_NUMBER() OVER (ORDER BY IDPR DESC) as rownum
                                                from (select IDPR,NAMEPR,PRICE,BRIEFSUM,CREATEDATE,QUANTITY, 
							[DISCOUNT]=  CASE when DATEDIFF(MONTH,CREATEDATE,getdate())=3 then '20%'
							when DATEDIFF(MONTH,CREATEDATE,getdate())=4 then '25%'
							when DATEDIFF(MONTH,CREATEDATE,getdate())>=5 then '30%'
					      ...
					      when DATEDIFF(MONTH,CREATEDATE,getdate())>=5 then CAST(PRODUCT.PRICE-(PRODUCT.PRICE*0.3) AS decimal(7,0))
                             ...
                             from PRODUCT 
                             where QUANTITY>0) t  where t.DISCOUNT is not null) tem
                        ...
                        <div class="col-sx-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="product-grid-item">
                                <div class="product-element-top"> <a href="./PRO_DETAILS.php?IDPR=<?=$IDPR?>">
                                        <img  class="thumbnail" src="../IMAGES/IMG_PRO/<?=$NAMEIM?>" height="300vh" alt="<?=$NAMEPR?>">
                                    </a><span class="off product-label"><?=$DISCOUNT?> off</span></div>
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        ...
                        if (($current_page > 1) && ($total_page > 1) ){
                            echo '<li class="page-item"><a class="page-link" href="SPECIALPRICE.php?IDBR='.'&page=' . ($current_page - 1) . '">Prev</a></li> ';
                        }
                        ...
                        for ($i = 1; $i <= $total_page; $i++) {
                            ...
                            if ($i == $current_page) {
                                ...
                                echo ' <li class="page-item " aria-current="page"><a class="page-link"<a href="SPECIALPRICE.php?IDBR='.'&page=' . $i . '">' . $i . '</a></li> ';
                            }
                        }

                        // nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
                        if ($current_page < $total_page && $total_page > 1) {
                            echo '<li class="page-item"><a class="page-link"<a href="SPECIALPRICE.php?IDBR='.'&page=' . ($current_page + 1) . '">Next</a> </li> ';
                        }
                        ?>
                    </ul>
                </nav>
