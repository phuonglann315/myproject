...
if (isset($_SESSION['USERNAME'])) {
    if ($_SESSION['USERNAME'] === 'admin') {
        if (isset($_GET['search'])) {
            $query = "select count (USERNAME) as total from USERS
where concat(USERNAME,EMAIL, PHONE) like ? collate SQL_Latin1_General_CP1_CI_AI";
            $param = [
                "%{$_GET['search']}%"
            ];
            ...
            if ($total_records != 0) {
                ...
                $query = "select * from ( select *, ROW_NUMBER() OVER (ORDER BY USERNAME) as rownum
                                      from (select * from USERS )t
     where concat (USERNAME, EMAIL, PHONE) like ? collate SQL_Latin1_General_CP1_CI_AI)tem
        ...
        where USERNAME !='admin'";
                ...
                $end = ($start + $limit - 1);
                $query = "select * from ( select *, ROW_NUMBER() OVER (ORDER BY USERNAME) as rownum
                                      from (select * from USERS  where USERNAME!='admin' )t )tem where tem.rownum between $start and $end ";
                $stmt = $dao->DML($query);
            }
        }
        else {
            ...
            <div class="col-lg-2 col-md-3">
                ...
                <div class="my-account-menu mt-6">
                    <ul class="nav account-menu-list flex-column">
                        ...
                        <li><a class="active" href="#pills-user"><i class="flaticon-user-1"></i> User Management</a>
                        </li>
            ...
            <div class="col-lg-10 col-md-9">
                ...
                <div class="tab-pane fade show active" id="pills-user">
                    <div class="my-product product-wrapper mt-6">
                        <h4 class="account-title">All users</h4>
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)):
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $row["PHONE"]; ?></td>
                                        <td><?= $row["locktime"]; ?></td>
                                        <td><?= $row["STTUSER"] ? "Active" : "Blocked"; ?></td>
                                        <td><a href=USER_EDIT.php?USERNAME=<?= $row["USERNAME"] ?>>Edit</a><br/></td>
                                    </tr>
                                <?php
                                endwhile;
                                ?>
